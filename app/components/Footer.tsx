...
export const Footer = ({ data }: FooterProps) => {
  ...
  const email = useMemo(
    () => contactInfo?.email || "kontakt@terazchiny.pl",
    [contactInfo?.email],
  );
  const phone = useMemo(
    () => contactInfo?.phone || "+48 123 456 789",
    [contactInfo?.phone],
  );
  const facebookUrl = useMemo(() => socials?.facebook, [socials?.facebook]);
  const instagramUrl = useMemo(() => socials?.instagram, [socials?.instagram]);
